...
def main():
    ...
    year_now = datetime.now(ZoneInfo("Asia/Malaysia")).strftime("%Y")
    t.gen_text("GIF_OS Modular BIOS v1.0.11", 1)
    t.gen_text(f"Copyright (C) {year_now}, \x1b[31mErry Asri Softwares Inc.\x1b[0m", 2)
    ...
    t.toggle_show_cursor(True)
    t.gen_typing_text("erryasri", 3, contin=True)
    ...
    t.toggle_show_cursor(False)
    time_now = datetime.now(ZoneInfo("Asia/Malaysia")).strftime(
        "%a %b %d %I:%M:%S %p %Z %Y"
    )
